    ...
    <div class="elBG panel">
      ...
      >
        ...
        <div class="rowFields mx-auto row">
          <div class="col-lg-3">
            <p class="textLabel">User:</p>
          </div>
          <div class="col-lg-9">
            <model-list-select
              :list="users"
              v-model="sales.user_id"
              option-value="id"
              option-text="name"
              name="user_id"
              ...
              v-validate="'required'"
            ></model-list-select>
            <small class="text-danger pull-left" v-show="errors.has('user_id')">user is required.</small>
          </div>
        </div>
        ...
        <template slot="modal-footer" slot-scope="{}">
          ...
          <b-button size="sm" variant="danger" v-if="roles.delete_sales" @click="btnDelete()">Delete</b-button>
        </template>
      </b-modal>
      <!-- End modalEdit -->
...
export default {
  ...
  created() {
    ...
    this.items = this.$global.getSales();
  ...
  methods: {
    ...
    btnCreate() {
      this.$validator.validateAll().then(result => {
        if (result) {
          this.tblisBusy = true;
          this.sales.user_id_log = this.user.id;
          this.sales.user_name_log = this.user.name;
          this.$http
      ...
      this.$validator.validateAll().then(result => {
        if (result) {
          swal({
            ...
            if (update) {
              this.sales.user_id_update = this.user.id;
              this.sales.user_name_update = this.user.name;
              this.$http
                .put("api/Sales/" + this.sales.id, this.sales)
                .then(response => {
                  this.items = response.body;
                  this.$global.setSales(response.body);
      ...
      if (this.roles.delete_sales) {
        swal({
          ...
          if (willDelete) {
            ...
            this.tblisBusy = true;
            this.$http
              .delete("api/Sales/" + this.sales.id)
              .then(response => {
                this.$bvModal.hide("modalEdit");
                this.$global.setSales(response.body);
