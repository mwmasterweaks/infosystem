    ...
    <div class="elBG panel">
      <div class="panel-heading">
        <p class="elClr panel-title">
          ...
          <button
            v-b-modal="'modalAdd'"
            v-if="roles.create_package_type"
            type="button"
      ...
      <b-modal
        ...
        size="xl"
        title="Manage Activity Ticket type"
        @ok="handleOk"
      >
        <!-- form -->
        <div class="rowFields mx-auto row">
          ...
          <div class="col-lg-9">
            <input
              ...
              class="form-control"
              v-b-tooltip.hover
              title="Input the activity ticket type"
              placeholder="Activity Ticket type"
              v-validate="'required'"
        ...
        <template slot="modal-footer" slot-scope="{}">
          ...
          <b-button
            size="sm"
            variant="danger"
            v-if="roles.delete_package_type"
            @click="btnDelete()"
          >Delete</b-button>
        </template>
      </b-modal>
      ...
      <b-modal
        ...
        :footer-text-variant="' elClr'"
        size="lg"
        title="Activity Ticket Type Form"
        @ok="handleOk"
      >
        <!--Form-------->
        <div class="rowFields mx-auto row">
...
export default {
  ...
  methods: {
    load() {
      ...
      this.tblisBusy = true;
      this.$http.get("api/ActivityTicketType").then(function(response) {
    ...
    tblRowClass(item, type) {
      if (!item) return;
      else if (this.roles.update_package_type) {
    ...
    tblRowClicked(item, index, event) {
      if (this.roles.update_package_type) {
    ...
    btnCreate() {
      this.$validator.validateAll().then(result => {
        if (result) {
          ...
          this.activity.user_name = this.user.name;
          this.$root.$emit("pageLoading");
          this.$http
            .post("api/ActivityTicketType", this.activity)
            .then(response => {
              console.log(response.body);
        ...
        if (result) {
          swal({
            ...
            if (update) {
              ...
              this.$http
                .put(
                  "api/ActivityTicketType/" + this.activity.id,
                  this.activity
                )
                .then(response => {
                  console.log(response.body);
      ...
      if (this.roles.delete_package_type) {
        swal({
          ...
          if (willDelete) {
            ...
            this.activity.user_name = this.user.name;
            this.$http
              .delete("api/ActivityTicketType/" + this.activity.id)
              .then(response => {
                console.log(response.body);
                this.$bvModal.hide("modalEdit");
        ...
        swal({
          title: "Opss.",
          text: "You are not allow to delete a type",
          icon: "warning",
          buttons: true,
          dangerMode: true
        });
