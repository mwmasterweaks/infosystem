  ...
  <div class="mx-auto col-md-12">
    <div class="elBG panel">
      ...
      >
        ...
        <div class="rowFields mx-auto row">
          ...
          <div class="col-lg-9">
            <input
              ...
              class="form-control"
              v-b-tooltip.hover
              title="Input the name of the region"
        ...
        <div class="rowFields mx-auto row">
          <div class="col-lg-3">
            <p class="textLabel">RM:</p>
          </div>
          <div class="col-lg-9">
            <model-list-select
              :list="users"
              v-model="region.user_id_rm"
              option-value="id"
              option-text="name"
              name="user_id"
              ...
              v-validate="'required'"
            ></model-list-select>
            <small class="text-danger pull-left" v-show="errors.has('user_id')">RM is required.</small>
          </div>
        </div>

        <div class="rowFields mx-auto row">
          <div class="col-lg-3">
            <p class="textLabel">Visor:</p>
          </div>
          <div class="col-lg-9">
            <model-list-select
              :list="users"
              v-model="region.user_id_visor"
              option-value="id"
              option-text="name"
              name="user_id"
              placeholder="Select/Search a user..."
              v-validate="'required'"
            ></model-list-select>
            <small class="text-danger pull-left" v-show="errors.has('user_id')">Visor is required.</small>
          </div>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: "rm_name", label: "RM name", sortable: true },
        { key: "visor_name", label: "Visor name", sortable: true },
  ...
  created() {
    ...
    this.items = this.$global.getRegion();
  ...
  methods: {
    ...
    tblRowClass(item, type) {
      if (!item) return;
      else if (this.roles.update_region) {
    ...
    btnCreate() {
      this.$validator.validateAll().then(result => {
        if (result) {
          ...
          this.region.user_name = this.user.name;
          this.$http
      ...
      this.$validator.validateAll().then(result => {
        if (result) {
          swal({
            ...
            if (update) {
              this.$http
                .put("api/Region/" + this.region.id, this.region)
                .then(response => {
                  this.items = response.body;
                  this.$global.setRegion(response.body);
    ...
    btnDelete() {
      if (this.roles.delete_region) {
        swal({
          title: "Are you sure?",
          text: "Do you really want to delete this region permanently",
          icon: "warning",
          buttons: ["No", "Yes"],
          dangerMode: true
        }).then(willDelete => {
          if (willDelete) {
            ...
            this.tblisBusy = true;
            this.$http
              .delete("api/Region/" + this.region.id)
              .then(response => {
                this.$bvModal.hide("modalEdit");
                this.$global.setRegion(response.body);
                swal("Delete!", "", "success").then(value => {
                  this.items = response.body;
                  this.totalRows = this.items.length;
                  this.tblisBusy = false;
                });
        ...
        swal({
          title: "Opss.",
          text: "You are not allow to delete a Region",
          icon: "warning",
          buttons: true,
          dangerMode: true
        });
