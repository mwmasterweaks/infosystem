    ...
    <div class="elBG panel">
      <div class="panel-heading">
        <p class="elClr panel-title">
          ...
          <button
            v-b-modal="'modalAdd'"
            v-if="roles.create_business_type"
            type="button"
      ...
      >
        ...
        <div class="rowFields mx-auto row">
          ...
          <div class="col-lg-9">
            <input
              ...
              class="form-control"
              v-b-tooltip.hover
              title="Input the type of Business"
              placeholder="Type of Business"
              v-validate="'required'"
              v-model.trim="business_type.name"
        ...
        <template slot="modal-footer" slot-scope="{}">
          ...
          <b-button
            size="sm"
            variant="danger"
            v-if="roles.delete_business_type"
            @click="btnDelete()"
            >Delete</b-button
          >
        </template>
...
export default {
  ...
  methods: {
    ...
    tblRowClass(item, type) {
      if (!item) return;
      else if (this.roles.update_business_type) {
    ...
    tblRowClicked(item, index, event) {
      if (this.roles.update_business_type) {
    ...
    btnCreate() {
      this.$validator.validateAll().then(result => {
        if (result) {
          ...
          this.business_type.user_id = this.user.id;
          this.business_type.user_name = this.user.name;
          this.$http
            .post("api/BusinessType", this.business_type)
            .then(response => {
        ...
        if (result) {
          swal({
            ...
            if (update) {
              ...
              this.$http
                .put(
                  "api/BusinessType/" + this.business_type.id,
                  this.business_type
                )
                .then(response => {
                  this.items = response.body;
    ...
    btnDelete() {
      if (this.roles.delete_business_type) {
        swal({
          title: "Are you sure?",
          text: "",
          icon: "warning",
          ...
          if (willDelete) {
            ...
            this.tblisBusy = true;
            this.$http
              .delete("api/BusinessType/" + this.business_type.id)
              .then(response => {
                this.$bvModal.hide("modalEdit");
        ...
        swal({
          title: "Opss.",
          text: "You are not allow to delete a Business Type",
          icon: "warning",
          buttons: true,
          dangerMode: true
        });
