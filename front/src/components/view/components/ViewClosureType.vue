        ...
        <div class="rowFields mx-auto row">
          ...
          <div class="col-lg-9">
            <input
              ...
              class="form-control"
              v-b-tooltip.hover
              title="Input the type of Closure"
              placeholder="Type of Closure"
              v-validate="'required'"
              v-model.trim="closure_type.name"
    ...
    btnCreate() {
      this.$validator.validateAll().then(result => {
        if (result) {
          ...
          this.closure_type.user_id = this.user.id;
          this.closure_type.user_name = this.user.name;
          this.$http
            .post("api/ClosureType", this.closure_type)
            .then(response => {
              swal("Created", "", "success");
              this.closure_type.name = "";
        ...
        if (result) {
          swal({
            ...
            if (update) {
              ...
              this.$http
                .put(
                  "api/ClosureType/" + this.closure_type.id,
                  this.closure_type
                )
                .then(response => {
                  this.items = response.body;
      ...
      if (this.roles.rm) {
        swal({
          ...
          if (willDelete) {
            ...
            this.tblisBusy = true;
            this.$http
              .delete("api/ClosureType/" + this.closure_type.id)
              .then(response => {
                this.$bvModal.hide("modalEdit");
