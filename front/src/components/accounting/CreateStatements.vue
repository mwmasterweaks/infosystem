  ...
  methods: {
    ...
    tblRowClicked(item, index, event) {
      if (this.roles.update_package_type) {
        ...
        this.pack.id = item.id;
        this.pack.name = item.name;
      }
    },
    handleOk(bvModalEvt) {
      ...
      this.$validator.validateAll().then(result => {
        if (result) {
          swal({
            ...
            if (update) {
              this.$http
                .put("api/PackageType/" + this.pack.id, this.pack)
                .then(response => {
                  this.items = response.body;
                  this.$global.setPackageTypes(response.body);
      ...
      if (this.roles.delete_package_type) {
        swal({
          ...
          if (willDelete) {
            this.tblisBusy = true;
            this.$http
              .delete("api/PackageType/" + this.pack.id)
              .then(response => {
                this.$bvModal.hide("modalEdit");
                this.$global.setPackageTypes(response.body);
