<template>
  <div>
    <button
      v-b-modal="'mainModalLocations'"
      type="button"
      style="margin-top:-20px;"
      class="btn btn-success btn-labeled pull-right margin-right-10"
    >Manage Locations</button>
    <b-modal
      id="mainModalLocations"
      size="xl"
    ...
    >
      ...
      <div class="elBG panel">
        <div class="panel-heading">
          <p class="elClr panel-title">Add Location</p>
        </div>
        <div class="elClr panel-body">
          <div class="rowFields mx-auto row">
            <div class="col-lg-3">
              <p class="textLabel">Location name:</p>
            </div>
            <div class="col-lg-9">
              <input
                type="text"
      ...
      <div class="elBG panel">
        <div class="panel-heading">
          <p class="elClr panel-title">View Locations</p>
        </div>
        <div class="elClr panel-body">
          <div>
            <b-table
...
export default {
  ...
  methods: {
    ...
    btnAddDevice() {
      this.$validator.validateAll().then(result => {
        if (result) {
          ...
          this.$http
            .post("api/MacLocation", this.addDevice)
            .then(response => {
              this.device_items = response.body;
              swal("Added", "", "success");
              this.addDevice.name = "";
    ...
    UpdateDevice() {
      swal({
        ...
        if (update) {
          ...
          this.$http
            .put("api/MacLocation/" + this.editDevice.id, this.editDevice)
            .then(response => {
              this.device_items = response.body;
              swal("Update!", "", "success");
              this.$bvModal.hide("ModalEditLocation");
              this.tblisBusy = false;
            })
            .catch(response => {
              swal({
    ...
    OpenModalEditDevice(item, index, button) {
      ...
      this.$bvModal.show("ModalEditLocation");
    },
    delDevice(item, index, button) {
      swal({
        ...
        if (willDelete) {
          this.tblisBusy = true;
          this.$http
            .delete("api/MacLocation/" + item.id)
            .then(response => {
