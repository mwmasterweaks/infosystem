...
class ticket_group extends Model
{
    protected $fillable = [
        'user_id', 'updated_by', 'ticket_id', 'complain', 'findings', 'action', 'complaint_new',
        'bwmon', 'device', 'loss', 'downtime', 'cacti', 'rep_findings', 'rep_action', 'rebatable', 'remarks',
        'report', 'prev_status', 'Status_Ticket_id', 'technical_assigned', 'target_date', 'team_assigned',
    ...
    public function ticket_group_client()
    {
        return $this->hasMany(ticket_group_client::class,  'ticket_group_id', 'id');
    }
    ...
    {
        return $this->hasMany(ticket_remarks_log::class, 'ticket_id', 'id')->where("form_type", 'ticket_group')->latest();
    }
